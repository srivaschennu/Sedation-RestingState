function GroupGraphfMRI(listname,measure)

loadpaths
filepath = fmripath;
load(sprintf('%s/graphdata_%s_%s.mat',filepath,listname,measure));
...
for f = 1:2;
    for midx = 1:nmes
        ...
        for g = 1:2
            groupvals = squeeze(mean(graph{m,weiorbin}(grp(:,1) == 1 & grp(:,5) == g,f,trange),1));
            groupstd = squeeze(std(graph{m,weiorbin}(grp(:,1) == 1 & grp(:,5) == g,f,trange)))/sqrt(length(groupvals));

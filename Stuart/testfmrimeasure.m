function testfmrimeasure(listname,measure,scaleidx)

loadpaths
filepath = fmripath;
load(sprintf('%sgraphdata_%s_fMRI.mat',filepath,listname));
...
testdata = squeeze(mean(mean(graph{m,3}(:,scaleidx,trange,:),4),3));
hold all
scatter(hitrate(grp(:,1) == 3),testdata(grp(:,1) == 3) - testdata(grp(:,1) == testlevel));
lsline
...
[~,p] = ttest2(testdata(grp(:,1) == testlevel & grp(:,5) == 1),...
    testdata(grp(:,1) == testlevel & grp(:,5) == 2),[],[],'unequal');
fprintf('t = %.2f, p = %.4f.\n',stats.tstat,p);

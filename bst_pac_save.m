function [DirectPAC, LowFreqs, HighFreqs] = bst_pac(F, sRate, bandNesting, bandNested, isUseParallel, isUseMex)
% BST_PAC: Calculate the DirectPAC metric for all the input time series.
%
% USAGE:  sPAC = bst_pac(F, sRate, bandNesting, bandNested, isUseParallel=0, isUseMex=0)
% 
% INPUTS:
...
% === USING MATLAB SCRIPTS ===
    ...
    % Initialize local DirectPAC matrix for current signals
    DirectPAC = zeros(nSignals, nSignals, length(lfreq), length(hfreq));
    % Compute direct PAC index for each high-freq and low-freq pair
    if isUseParallel
        ...
        for chan = 1:size(AMP,1)
            fprintf('%d ',chan);
            for ihf = 1:length(hfreq)
                DirectPAC(chan,:,:,ihf) = reshape(sum( bst_bsxfun(@times, PHASE, AMP(chan,:,ihf)), 2 ), [nSignals, length(lfreq)]);
            end
        end
        fprintf('\n');
    end
...
DirectPAC = bst_bsxfun(@rdivide, DirectPAC, shiftdim( tmp2(:,ones(1,size(DirectPAC,3)),:), -1) );

...
function [R,mbase,timesout,freqs,Rbootout,Rangle, coherresout, alltfX, alltfY,Rbootall] = newcrossf(X, Y, frame, tlimits, Fs, varwin, varargin)
...
%             formula = [ 'abs(mean(sign(angle(arg1)-angle(arg2)),3));' ];
           ...
           case 'icoh', 
            inputdata = { alltfX alltfY }; % default
            formula = 'sum(imag(arg1 .* conj(arg2)), 3) ./ sqrt( sum(abs(arg1).^2,3) .* sum(abs(arg2).^2,3) );';
        ...
        if strcmpi(g.boottype, 'shuffle') | strcmpi(g.boottype, 'rand')
            [Rbootout Rbootall] = bootstat(inputdata, formula, 'boottype', g.boottype, 'label', 'coherence', ...
